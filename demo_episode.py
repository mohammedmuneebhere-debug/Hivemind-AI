...
def reconstruct_positions_from_obs(
    ...
    *,
    ...
    """
    Mock env obs layout:
      obs = [local_state_agent0 (local_state_dim=4), global_state_flat (n_agents*local_state_dim)]
    where each agent local_state is [x, y, vx, vy].
    """
    ...
    return local_all[:, :2].astype(np.float32)  # (n_agents, 2)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate a PPO multi-agent motion preview GIF + input/output trace")
        ...
        step_traces.append(
            {
                ...
                "action": action.tolist(),  # (n_agents, action_dim)
                "next": {
    ...
    # Initialize with the right number of points so Matplotlib color validation passes.
    ...
    def update(frame_idx: int):
        ...
        # Update trails (per agent)
        hist_arr = np.stack(history, axis=0)  # (time, n_agents, 2)
